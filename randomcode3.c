...
//end of function prototype list
   ...
   if(fptr == NULL)
   {
    ...
    int count=1;
    printf("\t\t Sl.no \t Category Name");
    while(getc(fptr)!=EOF)
    {
        printf("\n");
        fscanf(fptr,"%[^\t\n]s",&s);
      printf("\t\t %d)\t %-18s",count,s);printf("\n");
      count++;
      }
   fclose(fptr);
    FILE pptr;
        ...
        switch(c1)
        {
            case 1: case 2: case 3: case 4: {pptr = fopen("C:\\Users\\vidhya\\Downloads\\foo\\products.txt","r")
                    read();} break;
                    default: categ();
        }
